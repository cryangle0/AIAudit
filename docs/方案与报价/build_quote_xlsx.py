...
def create_aidui_quote():
    ...
    g.add_module(
        ...
        features=[
            {
                'name': '回归测试与单元测试',
                'description': '使用真实抖音账单跑回归，64+ 单元测试覆盖核心引擎',
                'value_flow': '价值：保障对账准确性 ≥ 95%\n流程：测试 → 修 bug → 复测',
                'days': 1, 'cost': 1500, 'priority': 'P0'
            },
            {
                'name': '部署上线',
                'description': '部署到客户指定环境（GitHub Pages / 内部服务器 / 私有 OSS），配置域名与 HTTPS',
    ...
    g.add_section('📋 项目说明', [
        ...
        '采用纯前端 React 方案，浏览器端解析 Excel + localStorage 持久化，零后端依赖，开箱即用。',
        '11 个平台（7 国内 + 4 海外）通过适配器模式统一处理，新增平台仅需 1 个映射文件，扩展性强。',
    ])
    ...
    g.add_section('🔧 技术栈', [
        ...
        '• Excel 解析与生成：SheetJS (xlsx)',
        '• 状态管理：自研 Hook（useReducer + localStorage）',
        '• 测试：vitest（64+ 单元测试，覆盖核心引擎）',
        '• 部署：纯静态资源，支持任意 CDN/Pages 服务',
    ])
    ...
    g.add_section('⚠️ 风险与待决策事项', [
        '• 客户需要在第 1 周提供 3-5 个真实账单样本以提前调试适配器',
        '• 海外平台账单字段需客户提供 1 份样本，否则用通用模板',
        '• 数据存储默认 localStorage（5MB 配额），如需更大或多端同步需追加后端 API',
        '• 角色/权限/备份目前为前端展示，生产级权限控制需后端配套',
    ])
